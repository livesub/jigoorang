...
-->
                                ...
                                @foreach($opts as $opt)
                                    ...
                                    <tr>
                                        <td class="cs">
                                            <div onclick="openmodal_001({{ $order_info->order_id }}, {{ $cart->id }})">{{ $return_story }}</div>
...
-->
                ...
                <div class="box_cont">
                    <table class="ord_detail_table3">
                        ...
                        <tbody>
                        @php
                            if($order_info->de_send_cost_free == 0) $send_cost = $order_info->de_send_cost + $order_info->od_send_cost + $order_info->od_send_cost2;
            ...
            <!-- 모달 시작-->
            <div class="modal_001 modal fade" id="return_popup">
            </div>
            <!-- 모달 끝 -->
...
<script>
    function return_pop(order_id, cart_id){
        $.ajax({
            type : 'get',
            url : '{{ route('ajax_return_popup') }}',
            data : {
                "order_id"  : order_id,
                "cart_num"  : cart_id,
            },
...
<script>
    ...
    function openmodal_001(order_id, cart_id) {
        openModal();
        document.querySelector('.modal.modal_001').classList.add('in');
        return_pop(order_id, cart_id);
    }
