...
<table border=1>
    ...
    <tr>
        <td>현재 주문상태 : {{ $ment }}</td>
        <td>주문일시 : {{ substr($order_info->od_receipt_time, 0, 16) }}11111 ({{ $CustomUtils->get_yoil($order_info->od_receipt_time) }})</td>
...
<table border=1>
    ...
    <input type="hidden" name="sct_status" id="sct_status">
        ...
        <tr>
            <td>
                <span onclick="window.open('{{ route('return_popup','order_id='.$order_info->order_id.'&cart_num='.$cart->id) }}', 'PopupWin', 'width=500,height=600');">{{ $return_ment }}</span>
            </td>

                @if($k == 0)
            <td rowspan="{{ $rowspan }}"><img src="{{ asset($image) }}"> {{ stripslashes(mb_substr($cart->item_name, 0, 20, 'utf-8')) }}</td>
            <td rowspan="{{ $rowspan }}">
