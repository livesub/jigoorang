...
<table border=1>
    <form name="ordertqailform" id="ordertqailform" method="post" action="">
    {!! csrf_field() !!}
...
<script>
    function status_change(status){
        ...
        if (confirm(msg + "\n\n선택하신대로 처리하시겠습니까?")) {
            ...
            //$("#ordertqailform").submit();

            var form_var = $("#ordertqailform").serialize();
            $.ajax({
...
//return false;
                    ...
                    if(data.message == 'all_cancel'){
                        ...
                        //location.href = "{!! route('ordertqail', 'order_id='.$order_info->order_id.'&'.$page_move) !!}";
                        return false;
                    }
