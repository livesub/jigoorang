                        ...
                        <div class="board">
                            <table>
                                ...
                                @endphp
                                    ...
                                    <tr>
                                        <td><input type="checkbox" name="chk_id[]" value="{{ $item_info->id }}" id="chk_id_{{ $item_info->id }}" class="mg00" onclick="click_ck({{ $item_info->id }},'{{ $item_info->item_name }}', '{{ $item_info->item_code }}')"></td>
                                        <td>{{ $virtual_num }}</td>
                                        <td class="cate_name">
                                            <div>{{$ca_name_hap}}
                                            </div>
...
<script>
    function click_ck(id, item_name , item_code){
        var ment = "선택한 상품명 : " + item_name;
        $("#choice_name").html(ment);
        $("#exp_item_code").val(item_code);
        $("#exp_item_name").val(item_name);
        $("#item_id").val(id);
        closemodal_002();
