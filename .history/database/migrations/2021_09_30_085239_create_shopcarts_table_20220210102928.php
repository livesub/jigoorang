...
class CreateShopcartsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('shopcarts', function (Blueprint $table) {
            ...
            $table->integer('sct_qty_cancel')->default(0)->comment('취소전수량');
            ...
            $table->string('return_story')->comment('교환사유');
            $table->text('return_story_content')->comment('교환사유내용');
            $table->enum('return_process', ['N', 'Y', 'T'])->default('N')->comment('교환사유처리여부:N=>미처리,Y=>처리,T=>교환불가');
            $table->string('return_regi_date')->comment('교환신청일');
            $table->string('return_process_date')->comment('교환 처리일');
            $table->timestamps();
