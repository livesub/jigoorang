...
class CreateShopsettingsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('shopsettings', function (Blueprint $table) {
            ...
            $table->integer('tot_item_point')->default(0)->comment('전체 적용 포인트');

            $table->string('send_company_name')->nullable()->comment('배송업체명');
            $table->string('send_company_tel')->nullable()->comment('배송업체전화번호');
            $table->string('send_company_url')->nullable()->comment('배송 조회 URL');
            $table->timestamps();
        });
