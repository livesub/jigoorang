...
//$po_type = 적립금 지금 유형 : 1=>회원가입적립, 2=>상품 평가 적립, 3=>상품 평가 취소, 5=>평가단 평가 적립 ,6=>평가단 평가 취소, 7=>상품구매사용,8=>구매적립,9=>구매적립취소,10=>상품구매부분취소,11=>상품구매취소,12=>상품 포토 리뷰 적립, 13=>상품 포토 리뷰 취소, 14=>평가단 포토 리뷰 적립, 15=>평가단 포토 리뷰 취소 17=>지구랭 특별 적립, 18=>지구랭 특별 적립 취소
        ...
        $point_review = DB::table('shoppoints')->where([['order_id', $order_id], ['user_id', Auth::user()->user_id], ['po_type', 2]])->first();   //평가리뷰 포인트
        $point_photo = DB::table('shoppoints')->where([['order_id', $order_id], ['user_id', Auth::user()->user_id], ['po_type', 12]])->first();   //포토리뷰 포인트
