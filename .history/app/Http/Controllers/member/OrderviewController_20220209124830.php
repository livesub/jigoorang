...
//$success = true;
    ...
    public function return_sign_up(Request $request)
    {
        ...
        $order_info = DB::table('shoporders')->where([['order_id', $order_id], ['user_id', Auth::user()->user_id]])->first();
        $carts = DB::table('shopcarts')->where([['user_id', Auth::user()->user_id],['od_id', $order_info->order_id], ['id', $cart_num]])->first();
        ...
        $image = $CustomUtils->get_item_image($carts->item_code, 3);
        if($image == "") $image = asset("img/no_img.jpg");

        switch($carts->sct_status) {
            case "입금":
                $ment = "결제완료";
            break;
            case "준비":
