...
dd(count($point_review_sql));
        if(isset($point_review_sql)) $point_review = "aa";
/*
        if(count($point_review_sql) > 0) $point_review = $point_review_sql->po_point;

        $point_photo_sql = DB::table('shoppoints')->where([['order_id', $order_id], ['user_id', Auth::user()->user_id], ['po_type', 12]])->first();   //포토리뷰 포인트
        if(count($point_photo_sql) > 0) $point_photo = $point_photo_sql->po_point;
*/
