...
class QnaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($answer_type == "N"){
            $qna_list->where('qna_answer', null);
        }else if($answer_type == "Y"){
            $qna_list->where('qna_answer', '!=' , null);
        }

        if($qna_cate != ""){
            $qna_list->where('qna_cate', $qna_cate);
        }

        if($keyword != ""){
            $qna_list->where($user_type, 'like', '%'.$keyword.'%');
        ...
        $tailarr = array();
        $tailarr['answer_type'] = $answer_type;
        $tailarr['qna_cate'] = $qna_cate;
        $tailarr['user_type'] = $user_type;
        $tailarr['keyword'] = $keyword;

        $page_move = "page=".$page.'&answer_type='.$answer_type.'&qna_cate='.$qna_cate.'&user_type='.$user_type.'&keyword='.$keyword;
        ...
        $qna_answer_cnt = DB::table('qnas')->where('qna_answer', null)->count();
