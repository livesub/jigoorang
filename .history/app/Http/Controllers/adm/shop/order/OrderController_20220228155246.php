...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($od_status != "교환"){
            ...
            DB::statement("SET sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));");sss
        ...
        $page_move = "&page=$page&sel_field=$sel_field&search=$search&od_status=$od_status&fr_date=$fr_date&to_date=$to_date&order_type=$order_type";
        $sort_page_move = "&sel_field=$sel_field&search=$search&od_status=$od_status&fr_date=$fr_date&to_date=$to_date&order_type=$order_type";
...
*/
    ...
    public function ajax_order_check(Request $request)
    {
        $CustomUtils = new CustomUtils;
        ...
        $ment = "";
        if($check_type == "주문확인") $ment = "준비";
        else if($check_type == "결제완료") $ment = "입금";
    ...
    public function ajax_order_cancel(Request $request)
    {
        ...
        for($i = 0; $i < count($ct_chk); $i++){
            ...
            $request['imp_uid']                = $order_info->imp_uid;
            $request['merchant_uid']           = $order_info->order_id;  //order 테이블의 order_id 값
            $request['cancel_request_amount']  = $order_info->real_card_price;
            $request['reason']                 = "주문취소";
            $request['refund_holder']          = $order_info->od_deposit_name;
            $request['refund_bank']            = "";
            $request['refund_account']         = "";

            $this->ajax_itemspaycancel($request);
        }
    }

    public function ajax_order_send(Request $request)
    {
        $CustomUtils = new CustomUtils;

        $ct_chk     = $request->input('ct_chk');
        $od_invoice = $request->input('od_invoice');
    ...
    public function ajax_order_return(Request $request)
    {
        ...
        for($i = 0; $i < count($ct_chk); $i++){
            ...
            $order_up = DB::table('shoporders')->where('order_id', $order_info->order_id)->update([
                'od_status'     => "준비",
                'od_invoice'    => null,
                'od_invoice_date'   => null,
                'od_delivery_tel'   => null,
                'od_delivery_company'   => null,
                'od_delivery_url'   => null,
            ]);
        }
...
//$success = true;
    ...
    public function ajax_return_popup(Request $request)
    {
        $CustomUtils = new CustomUtils;
...
*/
    ...
    public function order_excel(Request $request)
    {
        ...
        if($order_sort == "" || $order_sort == "desc"){
            $order_rows = $orders->orderby('id', 'desc')->get();
        }else{
            $order_rows = $orders->orderby('id', 'asc')->get();
        }
        ...
        $file_name = "주문관리(".$ment.")".$now_date.".xls";
