...
var_dump("sdvsdv=====> ".$CustomUtils->get_cookie('de_cost_minus'));
            ...
            $aa = $hap_qty_price + $de_send_cost;
var_dump("aa====> ".$aa);
var_dump("hap_qty_price====> ".$hap_qty_price);

            if($card_price < ($hap_qty_price + $de_send_cost)){
var_dump("111111");
                $misu = $hap_qty_price + $de_send_cost;
                $od_cancel_price = $order_info->od_cancel_price + $misu; //취소금액
                $bb = ($hap_qty_price + $de_send_cost) - $hap_qty_price;
var_dump("bb====> ".$bb);
                //$CustomUtils->insert_point($order_info->user_id, ($hap_qty_price - $de_send_cost), '상품구매부분취소', 10,'', $order_id);
