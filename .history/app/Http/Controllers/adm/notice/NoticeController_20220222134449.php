...
class NoticeController extends Controller
{
    ...
    public function notice_write_save(Request $request)
    {
        ...
        if($request->hasFile('n_img'))
        {
            ...
            if(!$attachment_result[0])
            {
                ...
                for($k = 0; $k < 2; $k++){
                    $resize_width_file_tmp = explode("%%","360%%100");
                    $resize_height_file_tmp = explode("%%","180%%70");
    ...
    public function notice_modify_save(Request $request)
    {
        $CustomUtils = new CustomUtils;
        ...
        if($file_chk == 1){ //체크된 것들만 액션
            if($request->hasFile('n_img'))    //첨부가 있음
            {
                $n_img = $request->file('n_img');
                $attachment_result = CustomUtils::attachment_save($n_img,$path); //위의 패스로 이미지 저장됨
                ...
                if(!$attachment_result[0])
                {
                    return redirect()->route('adm.notice')->with('alert_messages', '첨부 파일이 잘못 되었습니다');
                    exit;
                }else{
                    for($k = 0; $k < 2; $k++){
                        $resize_width_file_tmp = explode("%%","360%%100");
        ...
        $update_result = Notices::find($id)->update($data);

        if($update_result) return redirect()->route('adm.notice','&page='.$page)->with('alert_messages', '수정 되었습니다');
        else return redirect('adm.notice')->with('alert_messages', '잠시 시스템 장애가 발생 하였습니다. 관리자에게 문의 하세요');  //치명적인 에러가 있을시
    }
