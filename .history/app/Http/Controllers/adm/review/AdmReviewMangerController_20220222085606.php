...
class AdmReviewMangerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($user_keyword != ""){
            $review_save_list = $review_save_list->where($user_keyword, 'like', '%'.$user_keyword.'%');
        }
/*
        if($seach_1 == "exp"){
...
*/
        ...
        $tailarr['review_type'] = $review_type;
        $tailarr['review_blind'] = $review_blind;
        $tailarr['user_info'] = $user_info;
        $tailarr['user_keyword'] = $user_keyword;
    ...
    public function ajax_item(Request $request)
    {
        ...
        $shopitems = DB::table('shopitems')->where('sca_id', $ca_id)->get();

        $html = "";
        foreach($shopitems as $shopitem){
            $selected = "";
            if($shopitem->item_code == $item_code) $selected = "selected";
            $html .= "
                <option value='".$shopitem->item_code."' ".$selected.">".$shopitem->item_name."</option>
            ";
        }
    ...
    public function review_excel_down(Request $request)
    {
        $CustomUtils = new CustomUtils;
