...
var_dump("1111111111====> ".$ordertemp_cnt);
exit;
...
exit;
    ...
    public function orderpayment(Request $request)
    {
        ...
        */
        ...
        //$ordertemp = shopordertemps::where([['order_id', $order_id], ['od_id', $od_id], ['user_id', Auth::user()->user_id]])->get();
        $ordertemp = DB::select("select count(*) as cnt from shopordertemps where order_id = '{$order_id}'");
var_dump("order_id-====> ".$order_id);            
var_dump("od_id-====> ".$od_id);            
var_dump("user-====> ".Auth::user()->user_id);            
dd($ordertemp[0]);
var_dump("od_cart_price=====".$ordertemp->od_cart_price);
        //예외 처리
        if(!$ordertemp){
