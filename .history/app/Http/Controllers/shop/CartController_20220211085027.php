...
class CartController extends Controller
{
    ...
    public function ajax_cart_register(Request $request)
    {
        ...
        if($sw_direct) $tmp_cart_id = $CustomUtils->get_session('ss_cart_direct');
            ...
            ->where([['a.od_id',$s_cart_id], ['a.user_id', Auth::user()->user_id], ['a.sct_status','쇼핑'], ['a.sct_direct','0']])  //장바구니 사라짐 문제
            //->groupBy('a.item_code')
            ->orderBy('a.id')
            ->get();
            ...
            for($i=0; $i<$count; $i++) {
                ...
                for($k=0; $k<$opt_count; $k++) {
                    ...
                    if(isset($sam_opt[0]->id) && $sam_opt[0]->id) {
                        ...
                        $up_result->sct_qty_cancel = $up_result->sct_qty_cancel + 1;
                        $result_up = $up_result->save();
    ...
    public function ajax_cart_qty_modify(Request $request)
    {
        ...
        $update_result = DB::table('shopcarts')->where('id', $cart_id)->update(['sct_qty' => $qty , 'sct_qty_cancel' => $qty]);
    }
