...
class ExpService
{
    ...
    public function set_page($page, $flag = 0, $ca_id, $exp_date_start, $keyword){
        ...
        $expList = DB::table('exp_list as a')
            ->select('a.*','b.sca_id', 'b.item_code', 'b.item_manufacture', 'b.item_name')
        ...
        if($flag == 0){
            ...
            if($exp_date_start != ""){
                ...
                $expList = $expList->where('a.exp_date_start', '>=', $exp_date_start);
            }
...
var_dump("1111111====> ".$item_keyword);
        ...
        if($item_search != ""){
            ...
            $search_sql = " AND a.sca_id = b.sca_id {$search_caid_sql} AND a.{$item_search} LIKE '%{$item_keyword}%' ";
        }else{
...
var_dump("222222222====> ".$item_keyword);
        $page_control = CustomUtils::page_function('shopitems',$pageNum,$writeList,$pageNumList,$type,$tb_name,$ca_id,$item_search,$item_keyword);
